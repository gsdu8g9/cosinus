...
class Plugin:
    ...
    def get_livecats():
        ...
        mew_ffp = subprocess.run(
            '''ffmpeg -i {input} -vframes 1 -c:v mjpeg -q:v 4 -f image2 pipe:1'''.format(input=mew_url),
    ...
    def __init__(self, bot):
        ...
        self.commands = {
            "/pony": self.get_pony,
            "/котик": self.get_cat,
            "/рептилия": self.get_reptile,
            "/лайвкоты": self.get_livecats
        }
    ...
    def call(self, event):
        if event.type != longpoll.VkEventType.MESSAGE_NEW:
            return
        try:
            (image_data, vk_response) = self.commands[event.text.partition(' ')[0]]()
        except KeyError:
            return
